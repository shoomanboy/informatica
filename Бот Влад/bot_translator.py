...
def handler(update:Update,context:CallbackContext):
    ...
    if update.message.text == rus_japan:
        src = "ru"  # src-исходный язык
        dest = "ja" # dest-язык назначения
        update.message.reply_text(text="Введите слово для перевода с русского на японский!",reply_markup=ReplyKeyboardRemove())
        return "translator_handler"

    if update.message.text==japan_rus:
        src="ja"    # src-исходный язык
        dest="ru"   # dest-язык назначения
        update.message.reply_text(text="Введите слово для перевода с японсого на расский!",reply_markup=ReplyKeyboardRemove())
        return "translator_handler"
    ...
    start_handler=updater.dispatcher.add_handler(
        ConversationHandler(entry_points=[CommandHandler("start",message_handler)],
                            states={
                                ...
                                "translator":[MessageHandler(Filters.regex("translator|japan->rus|rus->japan|список переведенных слов|/menu"),spisok_translator)],
                                "handler":[MessageHandler(Filters.regex("translator|japan->rus|rus->japan|список переведенных слов|/menu"),handler)],
